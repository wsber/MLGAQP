...
> The repository provides an extended **Technical Report** ([**`Technical_Report.pdf`**](./Technical_Report_0_99.pdf)) accompanying the main submission. Due to the strict 12-page conference format, the report collects complete mathematical derivations, extended system/cost analysis, and additional robustness discussions that are abbreviated or presented as proof sketches in the main paper.
> 
> **Key Contents of the Technical Report:**
> * **Rigorous Proof of Theorem 9 (AVG Consistency & Asymptotic Bias):** Complete proof under the joint asymptotic regime $K,m\rightarrow\infty$, establishing consistency for the true $\tau_{\mathrm{avg}}$ and asymptotic bias $\mathcal{O}(m^{-1})+\mathcal{O}(K^{-1})$, with the refined condition $m=o(K)$. The proof uses a two-stage error decomposition, conditional orthogonality, an exact ratio identity, and explicit control of the small-denominator event.
> * **Uniform Tree-Homomorphism Sampling:** Complete structural-induction proof showing that the top-down sampler generates every valid tree homomorphism uniformly with exact probability $P(h)=1/|\Omega|$, while preserving the complete valid structural-match space represented by the Candidate Space summary.
> * **WR Analysis and WOR Implementation:** Formalizes the distinction between **with-replacement (WR)** analysis and **without-replacement (WOR)** implementation. The report provides a rigorous Horvitz--Thompson proof of WOR unbiasedness, derives the corresponding variance expression involving first- and second-order inclusion probabilities, and explains why WR is used for tractable closed-form variance/allocation analysis while WOR is used in practice to avoid duplicate selections and improve finite-population sampling efficiency. A finite-population correction is also provided for the uniform-sampling case.
> * **End-to-End Cost and Latency Model:** Detailed decomposition of offline and online execution costs, including Candidate Space construction, $K$ tree-homomorphism sampling, proxy inference, and expensive Oracle verification. The benchmark reports approximately $800$--$1000$ ms preparation cost for $K=60{,}000$ tree samples, versus approximately $1.0$--$1.8$ ms per proxy item and $78$ ms per Oracle item in the evaluated NLI configuration.
> * **Theoretical Break-Even Analysis:** A conservative cost model comparing **PROXY** with an optimized exact engine, including single-predicate and multi-predicate cases. The report derives the speedup condition $S_o>\frac{1}{1-\alpha}$, and analyzes how preparation cost, projection cardinality, predicate ordering, selectivity, cache reuse, and Oracle cost determine the operational break-even boundary.
> * **Robustness to Uncalibrated Proxy Scores:** Documents that all reported experiments use raw, uncalibrated proxy outputs without post-hoc calibration. The report explains why calibration is not required for IPW-based unbiasedness under positive support, introduces defensive smoothing for zero/near-zero scores, and discusses the effect of proxy quality and calibration on sampling efficiency.
> * **Real-World Motivation and Case Study:** Provides an extended multimodal e-commerce scenario illustrating GA-ML workloads involving graph structure combined with text/image ML predicates and downstream COUNT/SUM/AVG business aggregation.
> * **Extended Reproducibility Details:** The report complements the public code repository with additional implementation assumptions, system-cost details, theoretical conditions, and experimental interpretation needed to reproduce and critically assess the reported results.
