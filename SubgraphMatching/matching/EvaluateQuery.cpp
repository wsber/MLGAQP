...
// global within this translation unit: control save behavior and file
static bool g_save_results = false;
static std::string g_result_file = "matching_results.csv";
static std::ofstream g_result_ofs;
static std::atomic<size_t> g_match_id{0};
static bool g_result_header_written = false;

// try to read environment variables at program start (constructor of static)
struct __ResultWriterInitializer {
    __ResultWriterInitializer() {
        const char* env = std::getenv("SAVE_RESULTS");
        if (env && std::string(env) != "0") {
            g_save_results = true;
            const char* f = std::getenv("RESULT_FILE");
            if (f && std::string(f).size() > 0) g_result_file = f;
            // open in truncate mode to clear previous run; will append later
            g_result_ofs.open(g_result_file, std::ios::out | std::ios::trunc);
            if (!g_result_ofs.is_open()) {
                // fallback: disable saving if file cannot be opened
                g_save_results = false;
            }
        }
    }
    ~__ResultWriterInitializer() {
        if (g_result_ofs.is_open()) g_result_ofs.close();
    }
} __resultWriterInit;

// helper to write one embedding (embedding indexed by query-vertex id 0..Q-1)
static inline void __write_embedding_to_csv(const ui *embedding, int qv_count) {
    if (!g_save_results) return;
    if (!g_result_ofs.is_open()) {
        // try reopen in append mode
        g_result_ofs.open(g_result_file, std::ios::out | std::ios::app);
        if (!g_result_ofs.is_open()) { g_save_results = false; return; }
    }
    if (!g_result_header_written) {
        g_result_ofs << "match_id";
        for (int i = 0; i < qv_count; ++i) g_result_ofs << ",u" << i;
        g_result_ofs << "\n";
        g_result_header_written = true;
    }
    size_t id = ++g_match_id;
    g_result_ofs << id;
    for (int i = 0; i < qv_count; ++i) {
        g_result_ofs << "," << embedding[i];
    }
    g_result_ofs << "\n";
    // flush periodically? keep buffered for performance (OS will flush on close)
}
    ...
    while (true) {
        while (idx[cur_depth] < idx_count[cur_depth]) {
            ...
            if (cur_depth == max_depth - 1) {
                embedding_cnt += 1;
                __write_embedding_to_csv(embedding, max_depth);
