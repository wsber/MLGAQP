...
def main():
    ...
    OUTPUT_DIR = f"/home/wangshuo/resource/AIModels/Finetune/NLI/base/{model_name}-binary-epoch{EPOCHS}/"
    ...
    class BinaryNLIDataset(Dataset):
        def __init__(self, df: pd.DataFrame, tokenizer, max_len: int = MAX_LEN):
            ...
            c = df["ML1_oracle1_contra"].values
            e = df["ML1_oracle1_entail"].values
            probs = (e / (c + e + 1e-8))
            # two-class soft labels: [p_contra, p_entail]
            self.soft_labels = torch.tensor(
                list(zip(1 - probs, probs)), dtype=torch.float32
            )
